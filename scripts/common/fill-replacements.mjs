...
const fillVar = async (key, envVars, environment) => {
  ...
  if (parentKey) {
    ...
    await fillVar(parentKey, envVars, environment);
  ...
  switch (varNameMapping[key].type) {
    case "serverless-domain":
      await fetchCli.fetchServerlessDomains();
      break;
    case "serverless-service":
      await fetchCli.fetchServerlessServices();
      break;
    case "serverless-environment":
      await fetchCli.fetchServerlessEnvironments(envVars[parentKey]);
      break;
    case "serverless-function":
      await fetchCli.fetchServerlessFunctions(envVars[parentKey]);
      break;
    case "studio-flow":
      await fetchCli.fetchStudioFlows();
      break;
    case "tr-queue":
      await fetchCli.fetchTrQueues(envVars[parentKey]);
      break;
    case "tr-workspace":
      await fetchCli.fetchTrWorkspaces();
      break;
    case "tr-workflow":
      await fetchCli.fetchTrWorkflows(envVars[parentKey]);
      break;
    case "sync-service":
...
const fillAllVars = async (envVars, account, environment) => {
  try {
    fetchCli.setAccount(account);
    ...
    for (const key in envVars) {
      ...
      if (process.env[key]) {
      ...
      } else if ((key == 'ACCOUNT_SID') && account.accountSid) {
        ...
        await fillVar(key, envVars, environment);
      }
    }
  } catch (error) {
...
export const fillReplacementsForString = async (data, account, environment) => {
  ...
  envVars = await fillAllVars(envVars, account, environment);
